...
@click.option("--mincoverage", default = 15, help = "Minimal signed edges to phase two variants [default: 15]")
@click.option("--conf", default = 0.9, help = "Minimal confidence to phase two variants [default: 0.9]")
@click.option("--maxlen", default = 10**6, help = "Maximal length to process [default: 10**7]")
...
@click.version_option(version = "0.1.0", prog_name = r"Fake haplotype phasing algorithm deceive most evalation metrics, based on Python 3.7+")
def main(invcf, bam, ref, output, threads, mincoverage, conf, maxlen, min_mapq, chrom, no_low_qual, verbose):
    ...
    with Pool(threads) as p:
        signed_edges = p.starmap(find_signed_edges, [(bam, maxlen, min_mapq, subject_variants[c], ref_len, chrom[c], ref, start_end_pos[c]) for c in range(len(chrom))])
