...
@click.version_option(version = "0.1.0-es", prog_name = r"Fake haplotype phasing algorithm deceive most evalation metrics, based on Python 3.10+")
def main(invcf, bam, ref, output, threads, mincoverage, conf, maxlen, min_mapq, no_low_qual, chrom, no_indel, verbose):
    ...
    with Pool(threads) as p:
        signed_edges = p.starmap(find_signed_edges, [(bam, maxlen, min_mapq, subject_variants[c], ref_len, chrom[c] ) for c in range(len(chrom))])

    # triangle consistent verify
    logging.info("Verfying signed edges")
    with Pool(threads) as p:
        phased_blocks = p.starmap(triangle_consistency, mincoverage, conf)

    for i in phased_blocks:
        pass
    sys.exit(0)
